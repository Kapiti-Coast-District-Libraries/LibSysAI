...
async function logUserRPD(userId: string) {
  const { error } = await supabaseClient.rpc('log_rpd', { uid: userId });
  if (error) {
    console.error('Failed to log RPD:', error.message);
  } else {
